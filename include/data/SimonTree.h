...
namespace SimonTree {
  struct Node {
    ...
    shared_ptr<Node> next = nullptr; // this is for pre-order traversal
  ...
  pair<shared_ptr<Node>, shared_ptr<Node>> splitNode(int i, shared_ptr<Node> root,
                                                   shared_ptr<Node> a);
  void fixTree(shared_ptr<Node> root, int n);
  void printTree(const shared_ptr<Node> &node, const string &w, 
